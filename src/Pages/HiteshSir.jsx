...
import { getHiteshSirChat } from "../Backend/hiteshsirchat.js";
...
function HiteshSir() {
    ...
    async function HResponse() {
        if (!input.trim()) return;

        const res = await getHiteshSirChat(input);
        ...
        <div className="h-[90vh] bg-gray-800 flex flex-col gap-y-[10px] pb-[10px]">
            ...
            <div className="h-[15%] px-[250px] flex gap-x-[10px]">
                ...
                <div className="h-[100%] w-[100%] flex justify-between items-center bg-white/10 px-[50px] rounded-2xl overflow-hidden">
                    <input value={input} onChange={(e) => setInput(e.target.value)} type="text" className="text-lg border-none outline-none w-[85%] border-white bg-transparent text-white" placeholder="Ask Hitesh Choudhary" />
                    <button onClick={HResponse}>
